<script lang="ts">
  ...
  const eventLoggedIn = "loggedin";
  const eventLoggedOut = "loggedout";
	...
	onMount(async () => {
		console.log(`GAVPresence.svelte# num chains ${data.chains.length}`);
		await init();
	});
	...
	const init = async () => {
		try {
			...
			if (web3auth.connected) {
				loggedIn = true;
        dispatch(eventLoggedIn, {provider});
			}
		} catch (error) {
			console.error(error);
		}
	...
	const logout = async () => {
		...
		loggedIn = false;
		provider = null;
    dispatch(eventLoggedOut, {provider: null});
	};
