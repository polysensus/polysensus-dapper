<script lang="ts">
	...
	async function connectCurrent() {
		...
		if (!address) {
			console.log(
				`PagePresence# no address for provider config: ${current.cfg.id} ${JSON.stringify(data.arenaAddress)}`
			);
			return;
		}
...
<ProvidersList on:providerSelected={providerSelected} {providers} bind:selected></ProvidersList>
